...
def dt_entropy(goal, examples):
    ...
    for count in counts:
        if count != 0:
            entropy -= (count / examples.shape[0]) * (np.log2(count / examples.shape[0]))
...
def dt_cond_entropy(attribute, col_idx, goal, examples):
    ...
    p_joint = np.zeros((len(attribute[1]), len(goal[1])))
    p_x = np.zeros(len(attribute[1]))

    for row in examples:
        p_x[row[col_idx]] += 1
        p_joint[row[col_idx], row[-1]] += 1

    p_x = p_x / examples.shape[0]
    p_joint = p_joint / examples.shape[0]

    for i in range(len(attribute[1])):
        for j in range(len(goal[1])):
            if p_joint[i, j] != 0:
                if p_x[i] == 0:
                    cond_entropy -= p_joint[i, j] * np.log2(p_joint[i, j])
                else:
                    cond_entropy -= p_joint[i, j] * (np.log2(p_joint[i, j] / p_x[i]))
...
def dt_intrinsic_info(attribute, col_idx, examples):
    ...
    for i in range(len(attribute[1])):
        if val[i] != 0:
            intrinsic_info -= (val[i]/examples.shape[0])*np.log2(val[i]/examples.shape[0])
...
def learn_decision_tree(parent, attributes, goal, examples, score_fun):
    ...
    if examples.size == 0:
    ...
    elif same_label(examples):
        node = TreeNode(parent, goal, examples, True, examples[0][-1])
        ...
        node = TreeNode(parent, attributes[index], examples, False, 0)
        atts = attributes.copy()
        del atts[index]

        for i in range(len(attributes[index][1])):
            ex_remove = []
            for j in range(examples.shape[0]):
                if examples[j][index] != i:
                    ex_remove.append(j)

            ex = np.delete(examples, ex_remove, 0)
            ex = np.delete(ex, index, 1)
            node.branches.append(learn_decision_tree(node, atts, goal, ex, score_fun))
