...
def value_iteration(env: mdp_env, agent: mdp_agent, eps: float, max_iter = 1000) -> np.ndarray:
    ...
    while iterations < max_iter:
        delta = 0
        for s in env.states:
            actions = np.sum(env.transition_model[s] * agent.utility, axis = 0)
            #print(actions, "current iter is ", iterations)
            old_utility = agent.utility[s].copy()
            agent.utility[s] = env.rewards[s] + agent.gamma * np.max(actions)
            policy[s] = np.argmax(np.sum(env.transition_model[s] * agent.utility))
            cur_delta = np.abs(agent.utility[s] - agent.utility)
            if np.abs(agent.utility[s] - old_utility) > delta:
                delta = np.abs(agent.utility[s] - old_utility)
            #print(sum)
        iterations += 1
